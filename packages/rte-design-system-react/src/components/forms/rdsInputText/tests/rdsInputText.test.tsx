...
import { RdsInputText, TextVariant } from '../RdsInputText.tsx';
...
describe('RdsInputText', () => {
  ...
  it('required should add "*" to the end of label', () => {
    const component = render(<RdsInputText label={TEST_LABEL} required value="" />);
    const input: HTMLInputElement = screen.getByLabelText(TEST_LABEL);
    expect(input).toHaveAttribute('required');
    const star = screen.getByText('*');
    expect(star).toBeInTheDocument();
    component.rerender(<RdsInputText label={TEST_LABEL} value="" />);
    expect(star).not.toBeInTheDocument();
  });

  const StatefulInputTextPassword = () => {
    const [value, setValue] = useState('');
    return <RdsInputText label={TEST_LABEL} value={value} password onChange={(e) => setValue(e)} />;
  };
