...
export const BANNER_COMMON_CONTAINER_CLASSES =
  'rds-flex rds-items-center rds-gap-2 rds-border-b-2 rds-px-3 rds-py-1 rds-min-h-6 rds-max-h-9';

export const BANNER_COMMON_TEXT_CLASSES = 'rds-line-clamp-3 rds-grow rds-text-body-xs';

export const BANNER_STATUS_CONTAINER_CLASSES = {
...
export const BANNER_STATUS_COLOR_CLASSES = {
...
export const bannerClassBuilder = (status: DisplayStatus) => ({
  containerClasses: clsx(BANNER_COMMON_CONTAINER_CLASSES, BANNER_STATUS_CONTAINER_CLASSES[status]),
  iconClasses: BANNER_STATUS_COLOR_CLASSES[status],
  textClasses: clsx(BANNER_COMMON_TEXT_CLASSES, BANNER_STATUS_COLOR_CLASSES[status]),
});
