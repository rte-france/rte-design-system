...
import StdInputDate, { StdInputDateProps } from "../StdInputDate";
...
const defaultInputSetup = (props: Omit<StdInputDateProps, "label">) => {
  ...
  const inputDate = screen.queryByRole<HTMLInputElement>("date");
  const inputTime = screen.queryByRole<HTMLInputElement>("time");
...
describe("StdInputDate", () => {
  ...
  it("renders the default timeOnly component", () => {
    const { inputDate, inputTime, label, helper } = defaultInputSetup({
      inputDateType: "timeOnly",
    });
    expect(document.querySelector(`#${TEST_ID}`)).toBeDefined();
    expect(inputDate).not.toBeDefined();
    expect(inputTime).toBeDefined();
    expect(label).toBeDefined();
    expect(helper).toBeDefined();
  ...
  it("renders the dateAndTime component as required", () => {
    const { inputDate, inputTime } = defaultInputSetup({
      inputDateType: "dateAndTime",
      required: true,
    });
  ...
  it("should call the onChange function when the event fires", () => {
    const { inputDate, inputTime, onChange } = defaultInputSetup({
      inputDateType: "dateAndTime",
    });
    ...
    expect(onChange).toHaveBeenNthCalledWith(
      1,
      new Date("1997-05-21T00:00:00")
    );
    fireEvent.change(inputTime!, { target: { value: "13:37" } });
    expect(onChange).toHaveBeenNthCalledWith(
      2,
      new Date("1997-05-21T13:37:00")
    );
  });
  ...
  it("should have the correct time precision hour", () => {
    const { inputTime } = defaultInputSetup({
      inputDateType: "dateAndTime",
      timePrecision: "hour",
    });
