...
import StdToggle from "../StdToggle";
...
const TEST_ICON = [
  { id: "add_icon_btn", value: "add", onChange: noop, content: StdIconId.Add },
  {
    id: "edit_icon_btn",
    value: "edit",
    onChange: noop,
    content: StdIconId.Edit,
  },
  {
    id: "delete_icon_btn",
    value: "delete",
    onChange: noop,
    content: StdIconId.Delete,
  },
];
...
describe("StdToggle", () => {
  it("renders the default StdToggle with texts", () => {
    render(<StdToggle id={TEST_ID} name={TEST_NAME} items={TEST_TEXT} />);
    const toggle = screen.getByRole("group");
    expect(toggle).toBeDefined();
    const items = screen.getAllByRole("radio");
    expect(items.length).toBe(3);
    expect(
      items.every((item, idx) => item.id === TEST_TEXT_ID_ARRAY[idx])
    ).toBe(true);
  });
  it("renders the default StdToggle with icons", () => {
    render(<StdToggle id={TEST_ID} name={TEST_NAME} items={TEST_ICON} />);
    const toggle = screen.getByRole("group");
    expect(toggle).toBeDefined();
    const items = screen.getAllByRole("radio");
    expect(items.length).toBe(3);
    expect(
      items.every((item, idx) => item.id === TEST_ICON_ID_ARRAY[idx])
    ).toBe(true);
  });
  it("renders the StdToggle with texts + disabled", () => {
    render(
      <StdToggle id={TEST_ID} name={TEST_NAME} items={TEST_TEXT} disabled />
    );
    const toggle = screen.getByRole("group");
    const items = screen.getAllByRole("radio");
    expect(
  ...
  it("renders the StdToggle with icons + disabled", () => {
    render(
      <StdToggle id={TEST_ID} name={TEST_NAME} items={TEST_ICON} disabled />
    );
    const toggle = screen.getByRole("group");
    expect(toggle).toBeDefined();
    const items = screen.getAllByRole("radio");
  ...
  it("renders the StdToggle with texts + default value", () => {
    render(
      <StdToggle
        id={TEST_ID}
        name={TEST_NAME}
        items={TEST_TEXT}
        defaultSelected={TEST_SELECTED_VALUE}
      />
    );
    const toggle = screen.getByRole("group");
    expect(toggle).toBeDefined();
    const items = screen.getAllByRole("radio");
    const checkedItem = items.filter(
      (item) => item.id === TEST_TEXT_ID_DEFAULT_VALUE
    );
  ...
  it("renders the StdToggle with icons + default value", () => {
    render(
      <StdToggle
        id={TEST_ID}
        name={TEST_NAME}
    ...
    const items = screen.getAllByRole("radio");
    const checkedItem = items.filter(
      (item) => item.id === TEST_ICON_ID_DEFAULT_VALUE
    );
    expect(checkedItem.length).toBe(1);
    expect(checkedItem[0]).toBeChecked();
    expect(
  ...
  it("renders the StdToggle with texts + default value + disabled", () => {
    render(
      <StdToggle
        id={TEST_ID}
        name={TEST_NAME}
  ...
  it("renders the StdToggle with icons + default value + disabled", () => {
    render(
      <StdToggle
        id={TEST_ID}
        name={TEST_NAME}
