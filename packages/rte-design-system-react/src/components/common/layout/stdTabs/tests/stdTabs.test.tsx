...
const TABS_GENERATOR_PARAMS_WITH_PRIMARY_ONLY = {
  primaryNumber: 10,
  secondaryNumber: 0,
};
const TABS_GENERATOR_PARAMS_WITH_SECONDARY = {
  primaryNumber: 10,
  secondaryNumber: 15,
};
...
describe("StdTabs", () => {
  it("renders StdTabs component with primary tablist only", () => {
    ...
    render(
      <StdTabs
        ...
        id={TEST_ID}
        renderPrimary={(item) => (
          <StdTabItem key={item.name} {...item} onClick={noop} />
        )}
      />
    );
  ...
  it("renders StdTabs component with primary tablist only with active", () => {
    ...
    expect(
      tabItems
        ...
        ?.children[0].className.includes(ACTIVE_CLASSES.active)
    ).toBe(true);
  });
  ...
  it("renders StdTabs component with primary and secondary tablist with active", () => {
    ...
    expect(
      tabItems
        .find((tab) => tab.id === TAB_ID_SECONDARY)
        ?.children[0].className.includes(ACTIVE_CLASSES.active)
    ).toBe(true);
  });
  ...
  it("renders StdTabs component with primary and secondary tablist without active (first default)", () => {
    ...
    render(
      <StdTabs
        ...
        id={TEST_ID}
        renderPrimary={(item) => (
          <StdTabItem key={item.name} id={item.id} {...item} onClick={noop} />
        )}
        renderSecondary={(item) => (
          <StdTabItem key={item.name} {...item} onClick={noop} />
        )}
  ...
  it("renders StdTabs component with primary tablist only with disabled", () => {
    ...
    expect(
      tabItems
        .find((tab) => tab.id === TAB_ID)
        ?.children[0].className.includes(DISABLED_CLASSES)
    ).toBe(true);
  });
    ...
    expect(
      tabItems.every(
        (tab) =>
          ...
          tab.children[0].className.includes(ACTIVE_CLASSES.inactive)
      )
    );
  });
});
...
describe("buildSecondaryTabsState function", () => {
  ...
  it("should return an object with default state when no secondary tabs are active", () => {
    ...
    expect(result).toEqual({ Tab1: "SubTab1" });
  });
});
