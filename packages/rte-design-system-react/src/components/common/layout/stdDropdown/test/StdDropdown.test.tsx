...
describe("StdDropdown", () => {
  ...
  it("properly renders the header section when header is passed", () => {
    render(
      <StdDropdownWithHeader
        id={TEST_ID}
        items={fakeDropdownShortList}
        header={TEST_HEADER}
      />
    );
    expect(screen.getByText(TEST_HEADER)).toBeDefined();
  });

  it("properly gives the container the combobox role if dropdown is multiple", () => {
    render(
      <StdDropdown id={TEST_ID} items={fakeDropdownShortList} isMultiple />
    );
  ...
  it("renders the correct amount of items in the item section with the proper content", () => {
    ...
    expect(screen.getAllByRole("option").length).toBe(
      fakeDropdownShortList.length
    );
    expect(
      document.querySelector(`#${fakeDropdownShortList[0].id}`)
    ).toBeDefined();
    expect(screen.getByText(fakeDropdownShortList[0].label)).toBeDefined();
  });
});
