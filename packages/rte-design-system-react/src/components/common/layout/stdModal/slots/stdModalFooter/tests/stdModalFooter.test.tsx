...
describe("StdModalFooter", () => {
  ...
  it("render the StdModalFooter with an information", () => {
    render(
      <StdModalFooter info={TEST_INFORMATION}>{TEST_BUTTON}</StdModalFooter>
    );
    expect(screen.getByRole("note")).toBeDefined();
    expect(screen.getByTitle(TEST_ICON_INFORMATION)).toBeDefined();
    expect(screen.getByText(TEST_TEXT_INFORMATION)).toBeDefined();
  });
});
