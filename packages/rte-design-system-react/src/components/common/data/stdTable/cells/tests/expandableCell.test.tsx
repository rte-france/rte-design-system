...
describe("expandableCell", () => {
  it("renders the row without child correctly", () => {
    ...
    const component = render(
      <ExpandableCell
        row={rowA}
        getValue={rowA.getVisibleCells()[0].getValue}
      />
    );
    expect(component.queryByRole("button")).not.toBeDefined();
  ...
  it("renders the row with child correctly", () => {
    ...
    const component = render(
      <ExpandableCell
        row={rowB}
        getValue={rowB.getVisibleCells()[0].getValue}
      />
    );
    expect(component.queryByRole("button")).toBeDefined();
    expect(component.getByTestId("spacer")).toHaveStyle("padding-left: 0rem");
  });
  it("renders the child row without child correctly", () => {
    ...
    const component = render(
      <ExpandableCell
        row={rowB1}
        getValue={rowB1.getVisibleCells()[0].getValue}
      />
    );
    expect(component.queryByRole("button")).not.toBeDefined();
