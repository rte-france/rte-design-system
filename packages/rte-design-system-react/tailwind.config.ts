...
import capitalizeFirst from './src/shared/plugins/tailwind/capitalizeFirst.plugin';

const bgImages = ['server-ok', 'server-ko', 'server-warn', 'update-notes'];

const backgroundImagridBasePath = '/background/imagrid/';
export default {
  darkMode: 'selector',
  ...
  theme: {
    ...
    backgroundImage: Object.fromEntries(
      bgImages.map((item) => [item, `url("${backgroundImagridBasePath}${item}.svg")`]),
    ),
    extend: {
      minWidth: {
        '3': '1.5rem',
  ...
  plugins: [
    ...
    plugin(({ addVariant }) => {
      addVariant(
        'no-desc-clickable-hovered',
        '&:not(:has(a:hover, input:hover, button:hover, select:hover, option:hover, .interactive:hover, [role="option"]:hover))',
      );
    }),
  ],
};
